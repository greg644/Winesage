...
export default function WineSage() {
...
```
  ...
  const i1start = t1.indexOf('[');
  const i1end = t1.lastIndexOf(']');
  if (i1start === -1 || i1end === -1) throw new Error('Could not read wines from image - try a clearer photo. Got: ' + t1.substring(0, 100));
  const wList = JSON.parse(t1.substring(i1start, i1end + 1));
  ...
  const i2start = t2.indexOf('[');
  const i2end = t2.lastIndexOf(']');
  if (i2start === -1 || i2end === -1) throw new Error('Step 2 failed. Got: ' + t2.substring(0, 200));
  setAnalysis(JSON.parse(t2.substring(i2start, i2end + 1)));
  setStatus(null);
} catch (err) {
...
<>
<Head>
<title>WineSage - Quality and Value Intelligence</title>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<meta name="description" content="Photograph any wine list. Instantly see quality ratings and retail markup analysis." />
</Head>
  ...
  <main style={{ maxWidth: 960, margin: '0 auto', padding: '40px 20px' }}>
    ...
    <header style={{ textAlign: 'center', marginBottom: 48 }}>
      ...
      <p style={{
        ...
        Quality and Value Intelligence
      </p>
      <p style={{
        fontSize: '0.8rem', color: 'var(--text-mid)',
        marginTop: 16, maxWidth: 460, margin: '16px auto 0', lineHeight: 1.8,
      }}>
    ...
    >
      ...
      {preview ? (
        <div>
          <img src={preview} alt="wine list" style={{
        ...
        <div>
          ...
          <div style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '1.3rem', color: 'var(--text-mid)', marginBottom: 6 }}>
    ...
    {wines && analysis && (
      <div style={{ animation: 'fadeIn 0.4s ease' }}>
        <div style={{
          ...
          Quality and Value Analysis - {wines.length} wines
        </div>
          ...
          <table ref={tableRef} style={{
            ...
            <tbody>
              {wines.map((w, i) => {
                ...
                const menuPrice = w.price_bottle ? 'GBP' + w.price_bottle : w.price_glass ? 'GBP' + w.price_glass + '/glass' : '-';
                const retail = a.retail_price ? '~GBP' + a.retail_price : '-';
                ...
                return (
                  <tr key={i} style={{ background: isBest ? '#1f1a09' : 'transparent' }}>
                    ...
                    <td style={{ padding: '12px 14px', borderBottom: '1px solid var(--surface2)', color: 'var(--text-mid)', whiteSpace: 'nowrap' }}>{menuPrice.replace('GBP', '\u00a3')}</td>
                    <td style={{ padding: '12px 14px', borderBottom: '1px solid var(--surface2)', color: 'var(--text-mid)', whiteSpace: 'nowrap' }}>{retail.replace('GBP', '\u00a3')}</td>
